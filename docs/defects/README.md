...
`password authentication failed` — was produced in this project by three unrelated causes, and not
one of them was a wrong password. It is also the only defect here that closed with a *guard* rather
than a fix: the report could not prevent a recurrence, so the suite now refuses to reset a target
